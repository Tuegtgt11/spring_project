...
//    private Logger
    ...
    @Transactional
    public List<Category> getAll(Specification<Category> specification) throws ApiException{
    ...
    public ResponseEntity<BaseResponse> findById(Long id) throws ApiException{
        ...
        if (optionalCategory.isPresent()){
            return ResponseEntity.status(HttpStatus.OK).body(
                    new BaseResponse(200, "hellooooooo", optionalCategory)
            );
        } else {
    ...
    public BaseResponse updateCate(Long id, CategoryRequest request) throws ApiException{
        validateRequestCreateException(request);
        if (!request.checkIsRoot()){
            // validate parent co ton tai khong
        ...
        Optional<Category> optionalCategory = categoryRepository.findById(id);
        Category existingCate = optionalCategory.get();
        existingCate.setName(request.getName());
        existingCate.setDescription(request.getDescription());
        existingCate.setIcon(request.getIcon());
        existingCate.setIsRoot(request.checkIsRoot() ? Constant.ONOFF.ON : Constant.ONOFF.OFF);
        categoryRepository.save(existingCate);
        if (!request.checkIsRoot()){
            // tao quan he

            CategoryRelationship categoryRelationship = new CategoryRelationship();
            CategoryRelationship.PK pk = new CategoryRelationship.PK(request.getParentId(), existingCate.getId());
            categoryRelationship.setPk(pk);
            categoryRelationshipRepository.save(categoryRelationship);
        }
